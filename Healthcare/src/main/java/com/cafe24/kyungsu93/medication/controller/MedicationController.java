...
import com.cafe24.kyungsu93.medication.service.MedicationDao;
...
public class MedicationController {
	...
	public String getMedicationContent(MedicationRequest medicationRequest, Model model
										...
										,HttpSession session) {
		logger.debug("MedicationController.addMedication POST 방식 호출");
		if(session.getAttribute("memberSessionNo").toString().equals(medicationService.getMedicationContent(medicationRequest).getMemberNo())) {
			model.addAttribute("currentPage", currentPage);
			model.addAttribute("pagePerRow", pagePerRow);
