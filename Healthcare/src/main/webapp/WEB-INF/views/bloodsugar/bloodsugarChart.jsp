...
/* 	var memberNo = $('#memberNoSession'); */
	...
	request.done(function( msg ) {
		...
		console.log(msg);
		if(msg.length<2){
			$('#chartResult').html('최근 일주일동안 등록된 혈당이 2개 미만입니다. 일주일 평균 혈당 차트를 보고싶으시면 혈당을 더 등록해주세요.');
			$('#addBtn').show();
		...
		var chartLineCount = 10;
			function drawChart() {
				...
				for(var i=0; i < msg.length; i++){
					data.addRow([new Date(msg[i].bloodsugarDate),parseInt(msg[i].fastingbloodsugar),(msg[i].fastingstate)]);
				}
				//그래프 옵션 설정
				var options = {
					//차트 상단의 제목 설정
					chart:{title : '혈당 그래프',
						subtitle: '최근 일주일 혈당 수치 그래프'	
					},
					//차트 크기 설정
					width: 900, // 넓이 옵션
					height: 500, // 높이 옵션
					...
					axes: {
						y: {
							diastolicPressure: {label: '혈당수치[fastingbloodsugar]'},
							systolicPressure: {label: '공복여부[fastingstate]'}
						}
					},
                      hAxis: {
